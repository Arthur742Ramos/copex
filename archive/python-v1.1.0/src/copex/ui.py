...
class TerminalInfo:
    """Cached terminal detection results."""
    _cached: dict[str, Any] | None = None
    
    @classmethod
    def detect(cls) -> dict[str, Any]:
        """Detect terminal capabilities and theme. Results are cached."""
        if cls._cached is not None:
            return cls._cached
        
        cls._cached = {
            "is_tty": sys.stdout.isatty() if hasattr(sys.stdout, 'isatty') else False,
            "is_light": _detect_light_theme(),
            "supports_true_color": _supports_true_color(),
            "supports_256_color": _supports_256_color(),
...
def _detect_light_theme() -> bool:
    ...
    """
    # 1. Check explicit COPEX_THEME environment variable
    copex_theme = os.environ.get("COPEX_THEME", "").lower()
    if copex_theme in ("light", "light-mode"):
        return True
    if copex_theme in ("dark", "dark-mode"):
        return False
    
    # 2. Check COLORFGBG (format: "fg;bg" - common on many terminals)
    colorfgbg = os.environ.get("COLORFGBG", "")
    if ";" in colorfgbg:
        try:
            _, bg = colorfgbg.rsplit(";", 1)
            bg_idx = int(bg)
            # In 16-color mode: 0=black, 7=white, 15=bright white
            if bg_idx in (7, 15):
                return True
            if bg_idx == 0:
                return False
    ...
    if term_program == "Apple_Terminal":
        ...
        # If COLORFGBG wasn't set and it's Apple Terminal, assume light
        if not colorfgbg:
            return True
    
    # 4. iTerm2 profile name detection
    iterm_profile = os.environ.get("ITERM_PROFILE", "").lower()
    if iterm_profile:
        if any(x in iterm_profile for x in ("light", "solarized light", "github light")):
            return True
        if any(x in iterm_profile for x in ("dark", "solarized dark", "dracula", "monokai")):
            return False
    
    # 5. VS Code terminal detection
    if os.environ.get("VSCODE_INJECTION") or os.environ.get("TERM_PROGRAM") == "vscode":
        ...
        if "light" in vscode_theme:
            return True
        # VS Code sets TERM_PROGRAM but we can check color scheme
        vscode_color = os.environ.get("VSCODE_TERMINAL_COLOR_SCHEME", "").lower()
        if "light" in vscode_color:
            return True
    
    # 6. Windows Terminal - check WT_PROFILE_ID or WT_SESSION
    if os.environ.get("WT_SESSION"):
        # Windows Terminal - could add more specific detection
        pass
    
    # 7. Kitty terminal
    if os.environ.get("KITTY_WINDOW_ID"):
        # Kitty can be queried but we'd need async detection
        pass
    
    # 8. Konsole
    if os.environ.get("KONSOLE_DBUS_SESSION"):
        konsole_profile = os.environ.get("KONSOLE_PROFILE_NAME", "").lower()
        if "light" in konsole_profile:
            return True
    
    # 9. GNOME Terminal
    if os.environ.get("GNOME_TERMINAL_SCREEN"):
        # Could check dconf but that's complex
        pass
    
    # 10. Generic BACKGROUND_COLOR hint
    bg_color = os.environ.get("BACKGROUND_COLOR", "").lower()
    if bg_color:
        if bg_color in ("white", "light", "#fff", "#ffffff"):
            return True
        if bg_color in ("black", "dark", "#000", "#000000"):
            return False
...
def _supports_256_color() -> bool:
    ...
    term = os.environ.get("TERM", "")
    return "256color" in term or _supports_true_color()
...
def is_light_theme() -> bool:
    """Detect if terminal is using a light theme."""
    return TerminalInfo.detect()["is_light"]


def supports_true_color() -> bool:
    """Check if terminal supports 24-bit RGB colors."""
    return TerminalInfo.detect()["supports_true_color"]


def supports_256_color() -> bool:
    """Check if terminal supports 256 colors."""
    return TerminalInfo.detect()["supports_256_color"]


def get_recommended_theme() -> str:
    """Get recommended theme based on terminal detection."""
...
THEME_PRESETS = {
    ...
    # Enhanced themes for 256-color terminals
    "dark-256": {
        # Optimized for dark terminals with 256 colors
        "PRIMARY": "color(39)",  # Deep sky blue
        "SECONDARY": "color(33)",  # Dodger blue
        "ACCENT": "color(170)",  # Orchid
        "SUCCESS": "color(78)",  # Sea green
        "WARNING": "color(220)",  # Gold
        "ERROR": "color(196)",  # Red
        "INFO": "color(39)",
        "REASONING": "color(244) italic",  # Grey
        "MESSAGE": "default",
        "CODE": "color(252)",  # Light grey
        "MUTED": "color(244)",
        "BORDER": "color(238)",  # Dark grey
        "BORDER_ACTIVE": "color(39)",
        "HEADER": "bold color(39)",
        "SUBHEADER": "bold",
    },
    "light-256": {
        # Optimized for light terminals with 256 colors
        "PRIMARY": "color(25)",  # Dark blue
        "SECONDARY": "color(31)",  # Steel blue
        "ACCENT": "color(127)",  # Deep purple
        "SUCCESS": "color(28)",  # Forest green
        "WARNING": "color(172)",  # Dark orange
        "ERROR": "color(160)",  # Dark red
        "INFO": "color(25)",
        "REASONING": "color(243) italic",
        "MESSAGE": "default",
        "CODE": "color(235)",  # Very dark grey
        "MUTED": "color(243)",
        "BORDER": "color(250)",  # Light grey
        "BORDER_ACTIVE": "color(25)",
        "HEADER": "bold color(25)",
        "SUBHEADER": "bold",
    },
    ...
    # Solarized themes
    "solarized-dark": {
        "PRIMARY": "color(37)",  # Cyan
        "SECONDARY": "color(33)",  # Blue
        "ACCENT": "color(136)",  # Yellow
        "SUCCESS": "color(64)",  # Green
        "WARNING": "color(166)",  # Orange
        "ERROR": "color(160)",  # Red
        "INFO": "color(37)",
        "REASONING": "color(245) italic",
        "MESSAGE": "color(254)",
        "CODE": "color(254)",
        "MUTED": "color(245)",
        "BORDER": "color(240)",
        "BORDER_ACTIVE": "color(37)",
        "HEADER": "bold color(37)",
        "SUBHEADER": "bold",
    },
    "solarized-light": {
        "PRIMARY": "color(33)",  # Blue
        "SECONDARY": "color(37)",  # Cyan
        "ACCENT": "color(136)",  # Yellow
        "SUCCESS": "color(64)",  # Green
        "WARNING": "color(166)",  # Orange
        "ERROR": "color(160)",  # Red
        "INFO": "color(33)",
        "REASONING": "color(244) italic",
        "MESSAGE": "color(235)",
        "CODE": "color(235)",
        "MUTED": "color(244)",
        "BORDER": "color(252)",
        "BORDER_ACTIVE": "color(33)",
        "HEADER": "bold color(33)",
        "SUBHEADER": "bold",
    },
    # GitHub-inspired themes
    "github-dark": {
        "PRIMARY": "color(75)",  # Light blue
        "SECONDARY": "color(141)",  # Light purple
        "ACCENT": "color(207)",  # Pink
        "SUCCESS": "color(114)",  # Light green
        "WARNING": "color(215)",  # Light orange
        "ERROR": "color(203)",  # Light red
        "INFO": "color(75)",
        "REASONING": "color(246) italic",
        "MESSAGE": "color(253)",
        "CODE": "color(253)",
        "MUTED": "color(246)",
        "BORDER": "color(240)",
        "BORDER_ACTIVE": "color(75)",
        "HEADER": "bold color(75)",
        "SUBHEADER": "bold",
    },
    "github-light": {
        "PRIMARY": "color(25)",  # Blue
        "SECONDARY": "color(91)",  # Purple
        "ACCENT": "color(125)",  # Deep pink
        "SUCCESS": "color(22)",  # Dark green
        "WARNING": "color(130)",  # Dark orange
        "ERROR": "color(124)",  # Dark red
        "INFO": "color(25)",
        "REASONING": "color(243) italic",
        "MESSAGE": "color(235)",
        "CODE": "color(235)",
...
def apply_theme(theme_name: str) -> bool:
    """
    Apply a theme preset to the Theme class.
    ...
    # Update the light/dark flag based on theme name
    Theme._is_light = "light" in theme_name.lower()
    return True


def get_theme_for_terminal() -> str:
    """
    Get the best theme for the current terminal.
    Uses auto-detection of light/dark mode and color capabilities.
    """
    info = TerminalInfo.detect()
    ...
    has_256 = info["supports_256_color"]
    
    # Check if specific terminal themes available
    term_program = info["term_program"].lower()
    
    # GitHub Codespaces / VS Code tends to use GitHub theme
    if "vscode" in term_program or os.environ.get("CODESPACES"):
        if is_light:
            return "github-light" if has_256 else "light"
        return "github-dark" if has_256 else "default"
    
    # iTerm with solarized profile
    iterm_profile = os.environ.get("ITERM_PROFILE", "").lower()
    if "solarized" in iterm_profile:
        return "solarized-light" if is_light else "solarized-dark"
    
    # Generic recommendation
    if is_light:
        return "light-256" if has_256 else "light"
    return "dark-256" if has_256 else "default"


def auto_apply_theme() -> str:
    """
    Automatically detect and apply the best theme.
...
def get_theme_preview(theme_name: str) -> str:
    """Get a preview of theme colors for display."""
    if theme_name not in THEME_PRESETS:
        return f"Theme '{theme_name}' not found"
    
    preset = THEME_PRESETS[theme_name]
    lines = [f"Theme: {theme_name}"]
    for attr in ["PRIMARY", "SECONDARY", "SUCCESS", "WARNING", "ERROR", "BORDER"]:
        color = preset.get(attr, "default")
        lines.append(f"  [{color}]{attr}: ████ sample text[/]")
    return "\n".join(lines)
...
class CopexUI:
    ...
    def _build_token_usage_panel(self) -> Panel | None:
        """Build a token usage panel with detailed stats."""
        if self.state.input_tokens == 0 and self.state.output_tokens == 0:
            return None
        ...
        return usage_display.to_panel(
            title=f"{self._icon_set.SPARKLE} Token Usage",
            border_style=Theme.BORDER,
        )
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # Streaming Context - Double-buffered Live Display
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    def streaming_context(
        ...
        *,
        ...
        """
        Create a streaming context for flicker-free live updates.
        
        Uses Rich's Live display with double-buffering and atomic updates
        to prevent flickering during rapid streaming.
        
        Usage:
            ui = CopexUI()
            with ui.streaming_context() as ctx:
                for chunk in stream:
                    ui.add_message(chunk)
                    ctx.refresh()  # Atomic update
        
        Args:
            refresh_per_second: Maximum refresh rate (default: 30 fps)
            transient: Clear display after context exits (default: True)
            vertical_overflow: How to handle overflow ("visible", "ellipsis", "crop")
        
        Returns:
            StreamingContext for use in a with statement
        """
...
class StreamingContext:
    """
    Context manager for flicker-free streaming output.
    
    Implements double-buffering with atomic updates:
    1. Content changes are buffered in CopexUI state
    2. refresh() builds a complete frame off-screen
    3. Frame is atomically swapped to display via Rich's Live
    
    Features:
    - Rate limiting to prevent excessive redraws
    - Dirty flag optimization (only redraw when content changes)
    - Graceful degradation for non-TTY environments
    - Automatic animation frame advancement
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._min_refresh_interval: float = 1.0 / refresh_per_second
        self._frame_count: int = 0
        self._is_tty: bool = ui.console.is_terminal
        
        # Buffer for building frames (double-buffer pattern)
        self._current_frame: Group | None = None
    
    def __enter__(self) -> "StreamingContext":
        """Start the live display context."""
        if self._is_tty:
            self._live = Live(
                self.ui.build_live_display(),
    ...
    def refresh(self, force: bool = False) -> bool:
        """
        Refresh the display if content has changed.
        
        Uses rate limiting and dirty-flag optimization to minimize redraws.
        
        Args:
            force: Force refresh even if rate limit or dirty check would skip
        
        Returns:
            True if display was actually refreshed, False if skipped
        """
        now = time.time()
        ...
        if not force and not self.ui.consume_dirty():
            return False
        
        # Build new frame (double-buffer: build off-screen)
        self._current_frame = self.ui.build_live_display()
        self._frame_count += 1
        
        # Atomic swap to display
        if self._live is not None:
            self._live.update(self._current_frame)
        elif not self._is_tty:
            # Non-TTY fallback: print plain text periodically
            if self._frame_count % 10 == 1:  # Every 10th frame
    ...
    def _print_plain_progress(self) -> None:
        """Print plain text progress for non-TTY environments."""
        # Simple progress indicator for non-interactive environments
        activity = self.ui.state.activity
        elapsed = self.ui.state.elapsed_str
        
        tool_count = len(self.ui.state.tool_calls)
        running = sum(1 for t in self.ui.state.tool_calls if t.status == "running")
        
        status = f"[{activity.value}] {elapsed}"
        if tool_count:
            status += f" | tools: {tool_count} ({running} running)"
        
        # Print to stderr to avoid polluting stdout
        print(status, file=sys.stderr)
    
    def finish(self, render_final: bool = True) -> None:
        """
        Finish streaming and optionally render final display.
        
        Args:
            render_final: Whether to print the final formatted output
        """
        # Force one last refresh
        self.refresh(force=True)
        
        # Exit live context if still active
        if self._live is not None:
        ...
        if render_final:
            self.ui.console.print(self.ui.build_final_display())
    ...
    def is_active(self) -> bool:
        """Whether the streaming context is active."""
